...
import TermsOfServiceModal from "../settings/termsofservice"; // <-- connected to your TOS file
...
export default function SignupScreen({ onBack, onSignupSuccess }) {
  ...
  const [showTermsModal, setShowTermsModal] = useState(false);  // controls TOS modal visibility
  const [hasAgreedToTerms, setHasAgreedToTerms] = useState(false); // shared agreed state — pass to Settings too
  ...
  "Secret Admirer 404",
  "Matcha Soulmate",
  "The Study-Date Specialist",
  "Lobby Lounge Legend",
  "Heart-Shaped Highlighter",
  "Campus Crush Catalyst",
  "Quad-Side Dreamer",
  ...
  "Flashcard Fanatic",
  "Lecture Hall Lyricist",
  "Pastel Pen-Pal",
  "Blueberry Syllabus",
  "Neon Notebook",
  "Glassmorphism Guru",
  "Retro Registrar",
  "The Aesthetic Auditor"
];
  ...
  "Estefania", "Felisa", "Granada", "Handumanan", "Mandalagan", 
  "Mansilingan", "Pahanocoy", "Paglaum", "Singcang-Airport", "Sum-ag", 
  ...
  // Opens the TOS modal first; success modal only shows after user agrees
  const handleCompleteSignup = () => {
    ...
    // Reset agreed state so user must check fresh each signup attempt
    setHasAgreedToTerms(false);
    setShowTermsModal(true);
  };

  // Called when user taps "I AGREE" inside TermsOfServiceModal
  const handleTermsAgree = () => {
    // hasAgreedToTerms is already true at this point (set via onCheckChange)
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/*
          ...
          - Imported from TermsOfServiceModal.js (your original file)
          - agreed={hasAgreedToTerms} — passes the current checked state into TOS
          - onCheckChange updates hasAgreedToTerms in SignupScreen
          - onAgree fires handleTermsAgree which then opens the success modal
          - Pass hasAgreedToTerms into onSignupSuccess so your Settings screen
            can read it and show the checkbox as already checked
        */}
        <TermsOfServiceModal
          ...
          onCheckChange={(val) => setHasAgreedToTerms(val)}
        />
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  onSignupSuccess({
                    ...
                    hasAgreedToTerms, // <-- your Settings screen reads this to reflect agreed checkbox
                  });
                  onBack();
                }}
              >
...
const styles = StyleSheet.create({
  ...
  modalOverlay: { flex: 1, backgroundColor: 'rgba(245, 247, 255, 0.95)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  confirmBox: {
    width: '90%', 
    maxWidth: 400,
    backgroundColor: '#fff', 
  ...
  verifyBadgeContainer: { marginBottom: 10 },
  outerCircle: { width: 70, height: 70, borderRadius: 35, borderWidth: 4, borderColor: '#000', justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff' },
  innerCircle: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#22c55e', borderWidth: 3, borderColor: '#000', justifyContent: 'center', alignItems: 'center' },
  checkIcon: { color: '#fff', fontSize: 28, fontWeight: '900' },
  confirmTitle: { fontSize: 22, fontWeight: '900', marginBottom: 15, color: '#000', textTransform: 'uppercase', letterSpacing: -0.5 },
  verificationSummary: { width: '100%', backgroundColor: '#f8fafc', padding: 15, borderRadius: 12, marginBottom: 15, borderWidth: 3, borderColor: '#000' },
  vLabel: { fontSize: 10, color: '#64748b', fontWeight: '900', marginBottom: 2 },
  vValue: { fontSize: 13, color: '#000', fontWeight: '900', marginBottom: 8 },
